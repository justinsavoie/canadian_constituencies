...
temp_data = pd.read_csv("data/file_for_doc2vec_v2.csv")
...
tagged_data = [TaggedDocument(words=_d, tags=[temp_data.speakerparty2[i],temp_data.speakername[i],temp_data.maintopic[i],temp_data.Province[i]]) for i, _d in enumerate(tokenized_corpus)]
...
wvsdf['word'] = words
wvsdf.to_csv("saved_models/word_embeddings_with_oral_questions_PROV.csv", index = False)
...
docsdf.to_csv("saved_models/doc_embeddings_with_oral_questions_PROV.csv", index = False)
